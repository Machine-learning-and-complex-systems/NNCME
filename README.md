...
Note: from some examples, there is another function with name "Mask" in its `.py` file, which is used to constrain the count of certain species. For example, DNA typically only has the count of 0 or 1 inside a cell. The "Mask" function allows only the reactions with such a proper count to occur.
