...
class GRU(BaseModel):
    """Representation of the GRU component.
    """
    ...
    def log_prob(self, x):
        ...
        if self.conservation>1:
            ...
            ids = x[:,0].long()
            log_probTemp= y.gather(1, ids.view(-1,1))[:,0]
            Mask1=remain>=0 
        ...
        if self.reverse:
            ...
            ids_rev = x[:,-1].long()
            log_prob_rev[:, 0] = y_rev.gather(1, ids_rev.view(-1,1))[:,0]
    ...
    def forward(self, x):
        ...
        batch_size = x.shape[0]#self.batch_size#x.shape[0]#
        # print('forward_batchsize:',batch_size)
        log_prob = torch.zeros_like(x)
        x_init = torch.zeros(batch_size, dtype=default_dtype_torch, device=self.device)
        if self.net_depth==1:
            h_init = torch.zeros(batch_size, self.net_width,dtype=default_dtype_torch, device=self.device)
        ...
        if self.conservation==1:
            h, y= self._forward(x_init, h_init,i)#self._forward2(x_init.reshape(-1,1), h_init,i)##TODO
            ids = x[:,0].long() 
            log_prob[:, 0] = y.gather(1, ids.view(-1,1))[:,0]#
        if self.conservation>1:
            remain=(self.conservation-x_init.detach()).long()
        ...
        if self.reverse:
            log_prob_rev = torch.zeros_like(x)
            h_rev, y_rev= self._forward(x_init, h_init,i)#self._forward2(x_init, h_init,i)
            ids_rev = x[:,-1].long()
            log_prob_rev[:, 0] = y_rev.gather(1, ids_rev.view(-1,1))[:,0]#y[:, 0] * mask[:, 0] + y[:, 1] * (1.0 - mask[:, 0])
        
        for i in range(1, self.n):
            if self.conservation==1:
                # print('\x1b[1;30;43m' ,'SPECIES',i,'\x1b[0m')
                # print(x[:, i - 1], h)
                h, y = self._forward(x[:, i - 1], h,i)#self._forward2(x[:, i - 1], h,i)##TODO
                ids = x[:,i].long()#x[:,i-1].long()#NewNew
                log_prob[:, i] = y.gather(1, ids.view(-1,1))[:,0]
            ...
            if self.reverse:
                h_rev, y_rev = self._forward(x[:, self.n-i ], h_rev,i)#self._forward2(x[:, self.n-i ], h_rev,i)
                ids_rev = x[:,self.n-i-1].long()#x[:,self.n-i].long()#NewNew
                log_prob_rev[:, i] = y_rev.gather(1, ids_rev.view(-1,1))[:,0]
