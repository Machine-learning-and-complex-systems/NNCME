...
def TransitionState3(sample,args,Tstep,net_new,ReactionMatLeft,V,cc,delta_T,model):
    # repeat=2
    # sample=sample.repeat(repeat,1)
    # argsbatch_size=args.batch_size*repeat
    
    """Transitionstate3 operation.

    

    Args:

        sample: Parameter forwarded to TransitionState3.

        args: Parameter forwarded to TransitionState3.

        Tstep: Parameter forwarded to TransitionState3.

        net_new: Parameter forwarded to TransitionState3.

        ReactionMatLeft: Parameter forwarded to TransitionState3.

        V: Parameter forwarded to TransitionState3.

        cc: Parameter forwarded to TransitionState3.

        delta_T: Parameter forwarded to TransitionState3.

        model: Parameter forwarded to TransitionState3.

    

    Returns:

        Result produced by TransitionState3.
    ...
    #R=torch.sum(Propensity_in,1)
    R=torch.sum(Propensity_out,1)
    Stay=1-torch.sum(Propensity_out,1)*args.delta_t

    # propensityAll=Propensity_out#torch.cat((Propensity_out,R.reshape(args.batch_size,1)),dim=1)
    # sampleAll=SampleNeighbor1D_Wout#torch.cat((SampleNeighbor1D_Wout,sample.view(args.batch_size,args.L,1)),dim=2)        
    # propensityAll=torch.cat((Propensity_out*args.delta_t,Stay.reshape(args.batch_size,1)),dim=1)
    # sampleAll=torch.cat((SampleNeighbor1D_Wout,sample.view(args.batch_size,args.L,1)),dim=2)
    if args.lossType=='ForwardKL2':
        propensityAll=Propensity_out#torch.cat((Propensity_out,R.reshape(args.batch_size,1)),dim=1)
        sampleAll=SampleNeighbor1D_Wout#torch.cat((SampleNeighbor1D_Wout,sample.view(args.batch_size,args.L,1)),dim=2)
    else:
        propensityAll=torch.cat((Propensity_out*args.delta_t,Stay.reshape(args.batch_size,1)),dim=1)
    ...
    values,indices=torch.max(torch.rand(args.batch_size,1).to(args.device)<torch.cumsum(propensityAll/torch.sum(propensityAll,dim=1).view(args.batch_size,1),dim=1),dim=1)
    sample2=[sampleAll.cpu()[j,:,indices[j]] for j in range(args.batch_size)]
    
    # sample2=torch.tensor([item.cpu().detach().numpy() for item in sample2]).view(args.batch_size,args.L)
    sample2=torch.cat(sample2).view(args.batch_size,args.L).to(args.device).detach()
    

    # # if Tstep:
    # print("**********************")
    # print(sample[0:5,:])
    # print(propensityAll[0:5,:]*100)
    # print(indices[0:5])
    # # print(sampleAll[0:5,:,:])      
    # print(sample2[0:5,:])      
    return sample2#LogTP_t1,delta_T,torch.log(Temp2),sample2

 
def TransitionState4(sample,args,Tstep,net_new,ReactionMatLeft,V,cc,delta_T,model):
    """Transitionstate4 operation.

    

    Args:

        sample: Parameter forwarded to TransitionState4.

        args: Parameter forwarded to TransitionState4.

        Tstep: Parameter forwarded to TransitionState4.

        net_new: Parameter forwarded to TransitionState4.

        ReactionMatLeft: Parameter forwarded to TransitionState4.

        V: Parameter forwarded to TransitionState4.

        cc: Parameter forwarded to TransitionState4.

        delta_T: Parameter forwarded to TransitionState4.

        model: Parameter forwarded to TransitionState4.

    

    Returns:

        Result produced by TransitionState4.
