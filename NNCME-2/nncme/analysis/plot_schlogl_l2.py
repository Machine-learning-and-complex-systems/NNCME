...
X1,X2,X3=0,1,2
legend=['X1', 'X2', 'X3']#['bA', 'cA', 'cB']#['a','b','c']#['A', 'B', 'C']#['a', 'b', 'c', 'A', 'B', 'C', 'bA', 'cA', 'cB']
color=['C0','C1','C2','C3','C4','C5','C6','C7','C8']
plotlist=[X1,X2]#[bA,cA,cB]#[a,b,c]#[A,B,C]#
species_num=3

out_filename = 'Schlogl/Schlogl2_nb1_Sites2_times5000_T1'
...
# data = np.load('Schlogl-Sites2-Para1-NADE-NatGrad-batch2000-epoch5-lr1-width16.npy', allow_pickle=True).item()
...
a=max(np.mean(SampleSum[:,:,X2],axis=1))+1
Range=[0,a]
...
markersize0=20
transparency=1-np.linspace(0, 0.99, NumPoints)
a=max(np.std(SampleSum[:,:,X2],axis=1))+1
Range=[0,a]
...
plt.rc('font', size=30)
legendsize = 30
probrange = [0.06, 0.06, 0.06]#[1,1,1]#[0.4, 0.3, 0.1]#
Range = [80, 80, 80]#[5,5,5]#[30, 30, 80]#
Tmax = max(TimePoins)
Tlist = [Tmax/10,Tmax/2,Tmax]#[90, 450, 900]
...
for j, T0 in enumerate(Tlist):
    ...
    h=ax.hist2d(SampleSum[T,:,X1],SampleSum[T,:,X2],bins=bins,cmap=newcmp,density=True,vmax=1e-3)#norm=mpl.colors.LogNorm(vmin=8e-5,vmax=2e-3))
